...
# ---- اکانت کاربر (آیدی + پسورد) با ذخیره روی فایل JSON، تا با ری‌استارت سرور از بین نره ----
...
GIFTCODES_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "giftcodes.json")
...
def save_giftcodes(data):
    tmp = GIFTCODES_FILE + ".tmp"
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False)
    os.replace(tmp, GIFTCODES_FILE)
...
NF_CHALLENGE_TIME = 15  # چند ثانیه بعد از هر دور، بقیه فرصت دارن به یه جواب مشکوک اعتراض کنن
NF_RESULT_PAUSE = 6   # بعد از رسیدگی به اعتراض‌ها، چند ثانیه نتیجه‌ی نهایی نمایش داده بشه
...
def nf_resolve_round(room):
    ...
    # امتیازها هنوز به جمع کل اضافه نمیشن؛ اول یه فرصت اعتراض داده میشه
    # (مثلاً جواب «آب پلو» تو دسته‌ی غذا معنی نداره، بقیه می‌تونن روش اعتراض بزنن)
...
def nf_resolve_room(room):
    ...
    if room["status"] == "playing":
        ...
        if room.get("round_result_until") and time.time() >= room["round_result_until"]:
            ...
            elif room["round"] >= room["target_rounds"]:
                best = max(room["scores"].values()) if room["scores"] else 0
...
def nf_matchmake():
    ...
    with nf_lock:
        ...
        for key, q in nf_mm_queues.items():
            q[:] = [p for p in q
                    if (p["id"] == uid and key == (n, target_rounds)) or (p["id"] != uid and now - p["ts"] < MM_STALE)]
        q = nf_mm_queues.setdefault((n, target_rounds), [])
        me = next((p for p in q if p["id"] == uid), None)
        if me:
            me["ts"], me["name"] = now, name
        else:
        ...
        del q[:n]
        code = nf_gen_code()
        room = nf_new_room(code, group[0]["id"], group[0]["name"], n, target_rounds)
        room["players"] += [{"id": p["id"], "name": p["name"]} for p in group[1:]]
        for p in group[1:]:
            room["scores"][p["id"]] = 0
        nf_rooms[code] = room
